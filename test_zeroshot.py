...
def validate(val_loader, classes, device, model, video_head, config, n_class, test_crops, test_clips):
    ...
    with torch.no_grad():
        ...
        for i, (image, class_id) in enumerate(val_loader):
            ...
            image_embedding, cls_embedding, text_embedding, logit_scale = model(image_input, text_inputs, class_id, return_token=True)
            cnt_time = time.time() - proc_start_time
            #similarity = video_head(image_features, text_features, cls_feature)  # b dim
            similarity = logit_scale * video_head(image_embedding.view(b, t, -1), text_embedding, cls_embedding)
            ...
            ########## gathering 
            i_features.append(concat_all_gather(image_embedding))
