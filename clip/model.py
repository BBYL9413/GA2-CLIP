...
class ResidualAttentionBlock_IVLP(nn.Module):
    ...
    #def forward(self, x: torch.Tensor, cts_input, cls_feat, layer_id):
    def forward(self, x: torch.Tensor):
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, 
                 dropout=None, prompts_needed = 0, text_layer=False, design_details=None):
        super().__init__()
        if dropout is None:
        ...
        self.width = width
        self.layers = layers
        self.return_text_VPT = text_layer
    ...
    def forward(self, x: torch.Tensor): 
        ...
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint.checkpoint(r, x,use_reentrant=False)
                
            else:
                x = r(x, use_reentrant=False)
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, 
                 output_dim: int, dropout = None, joint=False, emb_dropout = 0., T=8, design_details=False):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.prompts_needed = design_details["vision_depth"]
        ...
        ## Attention Blocks
        #self.transformer = SideTransformer(width, layers, heads, dropout=dropout, prompts_needed=self.prompts_needed, text_layer=False, design_details=self.design_details)
        self.transformer = Transformer(width, layers, heads, dropout=dropout, prompts_needed=self.prompts_needed, text_layer=False, design_details=self.design_details)
...
def build_model(state_dict: dict, design_details, tm=None,T=8,dropout=0., joint=False,emb_dropout=0.,pretrain=True):
